...
function LoginPage(){
  ...
  if(redirect){
    ...
    return(
        ...
        {action === "Sign Up" ? null : (
          <div className='forgot-password97'>Lost password? <span>Click Here!</span></div>
        )}
        <div className='submit-container97'>
        <button>
        <Link to={"/register"} className={action === "Login" ? "submit97 gray" : "submit97"} onClick={(e) => { if (action === "Login") { setAction("Sign Up")} else {register(e)}}}>Sign Up</Link>
        </button>
        <button>
        <Link to={"/login"} className={action === "Sign Up" ? "submit97 gray" : "submit97"} onClick={(e) => {if (action === "Sign Up") {setAction("Login")} else {login(e)}}}>Login</Link>
        </button>
